...
def rgb_name_modify():
    def file_number(file_name):
        match = re.search(r'_(\d+)\.jpg$', file_name)
        return int(match.group(1))

    directories = os.listdir(f"dataset/rgb")
    directories.sort()

    for item in directories:
        frames = [i for i in os.listdir(f"dataset/rgb/{item}")]
        sorted_frame = sorted(frames, key = file_number)
        
        item_number = int(item[-2:])
        # RGB3, RGB5~12 ,RGB15, RGB18,는 형광등 -> LED 정면 -> LED 좌 순으로 촬영
        if (4 < item_number and item_number < 13 and item_number != 7) or item_number == 3 or item_number == 15 or item_number == 18: 
            for idx, elem in enumerate(sorted_frame, start = 1):
                os.rename(f"dataset/rgb/{item}/{elem}", f"dataset/rgb/{item}/{item[-2:]}RGB{idx}.jpg")

        # RGB07, RGB13~는 LED 좌 -> LED 정면 -> 형광등 순으로 촬영
            ...
            for idx, elem in enumerate(sorted_frame, start = 1):
                ...
                os.rename(f"dataset/rgb/{item}/{elem}", f"dataset/rgb/{item}/{item[-2:]}RGB{file_order}.jpg")
                file_order += 1

def rgb_image_crop():
    target_image_dirs = os.listdir(f"dataset/rgb")
    
    for target_image_dir in tqdm(target_image_dirs):
        for target_image in os.listdir(f"dataset/rgb/{target_image_dir}"):
            target = cv2.imread(f"dataset/rgb/{target_image_dir}/{target_image}")
            H, W, _ = target.shape

            # print(H, W)
            # print(int(W/2 + W/4) - int(W/2 - W/7) + 1)
            
            crop_image = target[100:855,int(W/2 - W/7):int(W/2 + W/4),]

            H2, W2, _ = crop_image.shape

            # print(H2, W2)

            resized_crop_image = cv2.resize(crop_image, dsize = (256, 256), interpolation = cv2.INTER_LINEAR)

            H3, W3, _ = resized_crop_image.shape

            # print(H3, W3)

            cv2.imwrite(f"./test_image/{target_image}", crop_image)
